...
class AdminProductsController extends Controller
{
	...
	public function deleteproduct(Request $request){
		...
		$product = DB::table('products')->select('products_image')->where('products_id',$products_id)->first();
		if(file_exists($product->products_image)) unlink($product->products_image);

		$product_images = DB::table('products_images')->select('id','image')->where('products_id',$products_id)->get();
		if(count($product_images) > 0){
			foreach($product_images as $image){
				if(file_exists($image->image)) unlink($image->image);
			}
		}
		//end delete product images
